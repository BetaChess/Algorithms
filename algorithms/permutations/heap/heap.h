...
constexpr size_t heap_lchild_index(size_t i)
{
	return 2 * i + 1;
}

constexpr size_t heap_rchild_index(size_t i)
{
	return 2 * i + 2;
}

template<std::random_access_iterator I, std::sentinel_for<I> S>
constexpr bool heap_lchild_exists(const I it, S last, size_t i)
{
	return heap_lchild_index(i) < std::ranges::distance(it, last);
}

template<std::random_access_iterator I, std::sentinel_for<I> S>
constexpr bool heap_rchild_exists(const I it, S last, size_t i)
{
	return heap_rchild_index(i) < std::ranges::distance(it, last);
}

template<std::ranges::random_access_range R>
constexpr bool heap_lchild_exists(const R &range, size_t i)
{
	return heap_lchild_exists(std::ranges::begin(range), std::ranges::end(range), i);
}

template<std::ranges::random_access_range R>
constexpr bool heap_rchild_exists(const R &range, size_t i)
{
	return heap_rchild_exists(std::ranges::begin(range), std::ranges::end(range), i);
}
...
bool is_heap(const I it, S end, Comp comp = Comp{})
{
	...
	{
		if (heap_lchild_exists(it, end, i) && comp(it[heap_lchild_index(i)], it[i]))
			return false;
		if (heap_rchild_exists(it, end, i) && comp(it[heap_rchild_index(i)], it[i]))
			return false;
	}
	return true;
}
...
{
	return wmv::algorithms::is_heap(std::ranges::begin(range), std::ranges::end(range), comp);
}
...
void heapify(I it, S end, size_t i, Comp comp = Comp{})
{
	...
	size_t max = i;
	if (heap_lchild_exists(it, end, i) && comp(it[heap_lchild_index(i)], it[max]))
		max = heap_lchild_index(i);
	if (heap_rchild_exists(it, end, i) && comp(it[heap_rchild_index(i)], it[max]))
		max = heap_rchild_index(i);


	if (max != i)
	{
		std::ranges::iter_swap(it + i, it + max);
		heapify(it, end, max, comp);
	}
}
...
{
	wmv::algorithms::make_heap(std::ranges::begin(range), std::ranges::end(range), comp);
}
